...
from v2.models import AxisResult, RawFeatures, SkillsAssessment

logger = logging.getLogger("v2.skills_scorer")
...
_THRESHOLDS: dict[str, tuple[float, float, float, float]] = {
    ...
    # Intelligibility (post-correction values; +0.06 offset applied at extraction)
...
def score_intelligibility(raw: RawFeatures) -> AxisResult:
    ...
    The +0.06 Indian English instrument correction is already applied to
    intel_confidence at extraction time (transcriber.py). The threshold
    below is applied uniformly after correction.

    Feature:
      intel_confidence (100%): Bias-corrected Whisper token confidence
    """
    flags: list[str] = []
