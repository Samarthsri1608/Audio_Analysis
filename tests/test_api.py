...
class TestAPIContracts:
    ...
    def test_convert_endpoint_exists(self):
        """POST /convert_from_video must exist (not 404)."""
        resp = requests.post(f"{BASE_URL}/convert_from_video", timeout=10)
        assert resp.status_code != 404, "/convert_from_video not found (404)"
...
class TestAccuracySanity:
    ...
    def test_high_wpm_correlates_with_fluency(self):
        """A candidate speaking at ~100+ WPM should have fluency score > 1.5."""
        fp = TEST_FILES["wav"]
        if not fp.exists():
            pytest.skip()
        data = _evaluate(fp)
        wpm  = data["features"]["fluency"]["wpm"]
        fsco = data["scores"]["fluency"]
        if wpm >= 100:
            assert fsco > 1.5, \
                f"WPM={wpm:.0f} but fluency score is only {fsco:.2f} — scoring may be broken"

    def test_high_confidence_correlates_with_intelligibility(self):
        """mean_confidence > 0.80 should yield intelligibility score > 2.5."""
        fp = TEST_FILES["wav"]
        if not fp.exists():
            pytest.skip()
        data = _evaluate(fp)
        mc   = data["features"]["intelligibility"]["mean_confidence"]
        isco = data["scores"]["intelligibility"]
        if mc > 0.80:
            assert isco > 2.5, \
                f"mean_confidence={mc:.3f} but intelligibility score is only {isco:.2f}"

    def test_zero_grammar_errors_yields_top_language_control(self):
        """0 grammar errors should give language_control == 5.0."""
        fp = TEST_FILES["wav"]
        if not fp.exists():
            pytest.skip()
        data = _evaluate(fp)
        gec  = data["features"]["language_control"].get("grammar_error_count", -1)
        lsco = data["scores"]["language_control"]
        if gec == 0:
            assert lsco >= 4.9, \
                f"0 grammar errors but language_control score is only {lsco:.2f}"

    def test_score_ordering_matches_feature_ordering(self):
        """For two files, the one with higher WPM should likely have higher fluency."""
        ...
        fsco1  = d1["scores"]["fluency"]
        fsco2  = d2["scores"]["fluency"]
        if abs(wpm1 - wpm2) > 40:
            if wpm1 > wpm2:
                assert fsco1 >= fsco2 - 1.5, (
                    f"wav has WPM={wpm1:.0f} > mp4 WPM={wpm2:.0f} "
                    f"but fluency scores are reversed: {fsco1:.2f} vs {fsco2:.2f}"
                )
