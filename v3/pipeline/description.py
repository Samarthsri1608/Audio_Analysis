...
This is the ONLY place in V3 where a style-summary LLM is used. Everything else is
...
Updated for Zeko Unified Communication Framework v1:
  ...
  - Fallback description (when Azure config is missing) uses StyleProfile instead of just a label
...
from v3.models import ArchetypeBlend, CommunicationSignals, StyleProfile

logger = logging.getLogger("v3.description")
...
async def generate_description(
    ...
    """
    if not GEMINI_API_KEY:
        logger.info("Gemini API key not set — returning fallback description.")
